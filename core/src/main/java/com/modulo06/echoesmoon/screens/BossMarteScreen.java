...
public class BossMarteScreen implements Screen {
    ...
    public BossMarteScreen(Game game, GameSaveData save) {
        this.game = game;
        this.save = save;
        this.save.sincronizarInventario();
        camera.setToOrtho(false, WORLD_W, WORLD_H);
        boss = new BossMarte(560, 400);
        carregarTexturas();
        spawnAmbient();
    }

    private void carregarTexturas() {
        fundoTex = load("fundo_marte.png");
        bossTex = load("boss_marte.png");
    ...
    private void spawnAmbient() {
        Array<Rectangle> forbidden = new Array<>(); forbidden.add(boss.rect);
        WorldRock.spawnMany(pedrasMapa, 12, WORLD_W, WORLD_H, player, forbidden, 150f);
        for (int i = 0; i < 5; i++) {
    ...
    @Override public void render(float delta) {
        update(delta);
        Gdx.gl.glClearColor(0.45f, 0.12f, 0.05f, 1f);
        ...
        for (WorldRock rock : pedrasMapa) if (rockTex != null) { batch.setColor(0.70f, 0.45f, 0.32f, 1f); batch.draw(rockTex, rock.rect.x, rock.rect.y, rock.rect.width, rock.rect.height); batch.setColor(Color.WHITE); }
        ...
        for (SlashWave rock : pedrasBoss) if (rock.active && rockTex != null) batch.draw(rockTex, rock.rect.x, rock.rect.y, 16, 16, 32, 32, 1.25f, 1.25f, rock.angle, 0, 0, rockTex.getWidth(), rockTex.getHeight(), false, false);
    ...
    private void update(float delta) {
        if (!save.marteMissoesOk) { game.setScreen(new MarsScreen(game, save)); return; }
        ...
        for (int i = comidas.size - 1; i >= 0; i--) if (player.overlaps(comidas.get(i).rect)) { save.inventario.add("COMIDA"); comidas.removeIndex(i); mensagem = "+1 COMIDA"; mensagemTimer = 1.5f; }
        ...
        if (boss.consumeRockThrow()) pedrasBoss.add(new SlashWave(boss.rect.x + boss.rect.width / 2f, boss.rect.y + boss.rect.height / 2f, boss.getRockTarget().x, boss.getRockTarget().y));
        ...
        if ((Gdx.input.isButtonJustPressed(Input.Buttons.LEFT) || Gdx.input.isKeyJustPressed(Input.Keys.SPACE)) && cooldown <= 0f && save.inventario.municao > 0 && boss.ativo) {
            ...
            save.inventario.municao--; save.municao = save.inventario.municao;
            Vector3 mouse = new Vector3(Gdx.input.getX(), Gdx.input.getY(), 0f); camera.unproject(mouse);
        ...
        for (int i = tirosJogador.size - 1; i >= 0; i--) {
            ...
            if (boss.ativo && shot.rect.overlaps(boss.rect)) {
                ...
                shot.active = false;
                if (!boss.ativo) {
                    save.bossMarteDerrotado = true;
                    save.inventario.add("CHAVE_MARTE");
                    save.inventario.municao = 25; save.municao = 25;
                    save.fase = "TITA"; save.titaMissoesOk = true; save.salvar();
        ...
        for (int i = pedrasBoss.size - 1; i >= 0; i--) {
            SlashWave p = pedrasBoss.get(i); p.update(delta);
    ...
    private void desenharHUD() {
        ...
        font.draw(batch, "TITAN-FERRUGEM — BOSS MARTE", 24, 575);
        ...
        font.draw(batch, "I inventario | C comida | CLICK ataca", 24, 28);
        if (mensagemTimer > 0f) font.draw(batch, mensagem, 580, 30);
        batch.end();
